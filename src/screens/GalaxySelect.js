...
export default class GalaxySelect {
  ...
  _draw() {
    ...
    rg.addColorStop(0, 'rgba(16,24,72,0.15)');
  ...
  _drawNode(node, idx, t) {
    ...
    ctx.strokeStyle = isFocused ? 'rgba(255,255,255,0.25)' : 'rgba(200,220,255,0.1)';
    ctx.lineWidth = 0.8;
    ctx.stroke();
    ...
    ctx.fillStyle = isFocused ? 'rgba(255,255,255,0.8)' : 'rgba(200,220,255,0.4)';
    ctx.fill();

    // Glowing aura
    ctx.save();
    ctx.shadowColor = isFocused ? 'rgba(255,255,255,0.8)' : (isHov ? 'rgba(79,195,247,0.8)' : 'rgba(200,220,255,0.4)');
    ctx.shadowBlur  = isFocused ? 40 : 25;
    
    // Core star
    this._miniStar(node.x, node.y, r, t * (0.02 + idx * 0.01), isFocused);
    
    ctx.restore();
  }

  _miniStar(cx, cy, size, rot, isFocused) {
    ...
    ctx.closePath();
    ctx.fillStyle = isFocused ? 'rgba(255,255,255,0.95)' : 'rgba(200,220,255,0.8)';
    ctx.fill();
